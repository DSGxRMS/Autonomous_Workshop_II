# src/core/dijkstra_weighted_solution.py
#!/usr/bin/env python3
...
class DijkstraAlgo:
    ...
    final_total_cost: Optional[int] = None
    ...
    def _path_total_cost(self, path: List[Cell]) -> int:
        if not path or len(path) == 1:
            return 0
        total = 0
        # convention: cost to ENTER a cell = weight(cell); start contributes 0
        for cell in path[1:]:
            total += int(self.grid.cost_of(cell))
        return total
    ...
    def step(self) -> StepResult:
        ...
        if u == self.goal_cell:
            ...
            self.final_total_cost = self._path_total_cost(path) if path else None
        ...
        for v in self._neighbors4(u):
            try:
                ...
                continue  # should not happen due to is_block filter
    ...
    def _metrics(self, path_len: int = 0) -> dict:
        return {
            ...
            "path_len": path_len,
            "total_cost": self.final_total_cost,
        }
